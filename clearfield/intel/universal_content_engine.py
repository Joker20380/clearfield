...
Каждое фактическое предложение должно быть прямым пересказом одного или
нескольких evidence-тезисов. Если тезис лишь правдоподобен, но явно не указан
в evidence pack, не включай его.
Не достраивай цели применения, причины, последствия, риски, преимущества,
ограничения, подготовку, диагнозы, лечение и сценарии использования.
Не создавай сравнения вида «в отличие от», если обе стороны сравнения
не подтверждены. Не делай отрицательный вывод из отсутствия информации.
...
- Естественно используй главный поисковый запрос в первом абзаце; допускаются
  служебные слова и знаки препинания между его словами.
- Используй остальные ключевые фразы естественно, без повторов ради SEO.
- Не используй H1 внутри body_markdown.
- Не добавляй неподтверждённые причины, последствия и рекомендации.
- Перед ответом молча проверь каждое фактическое предложение: его полный
  смысл, включая отрицания и сравнения, должен следовать из указанных E-маркеров.
- Если evidence pack не позволяет заполнить раздел фактами, сократи раздел,
  а не дополняй его общими знаниями.
...
def build_factual_verification_prompt(brief, generated_body: str) -> str:
    evidence = "\n".join(
        (
            ...
            f"Тезис: {item.get('claim')}\n"
            f"Точная цитата: {item.get('source_quote') or 'не задана'}"
        )
        for item in brief.evidence_pack
        if isinstance(item, dict)
    )
...
Проверяй полный смысл предложения: отрицания, сравнения, цели применения,
причины, последствия, рекомендации и ограничения требуют отдельной прямой
опоры. Нельзя выводить факт только из отсутствия сведений.
Утверждение считается подтверждённым, если его прямо поддерживает хотя бы один
evidence-тезис. Совместимый более широкий диапазон в другом тезисе не отменяет
точное значение: например, «3 месяца» совместимо с диапазоном «2–3 месяца».
Считай конфликтом только взаимоисключающие сведения.
